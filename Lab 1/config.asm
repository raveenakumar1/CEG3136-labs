...
CFG_IX   DS.B 1   ; variable for index
CFG_INPUT DS.B 1  ; stores user input
CFG_FLAG DS.B 1   ; flag to indicate success/failure
CFG_VARSIZE:
CFB_PR_D	DS.W 1 ; saves register D
...
configCodes: pshd
      ...
      ; byte flag;
      leas -CFG_VARSIZE,SP
cfg_if1: jsr enterMstCode ; verify if master code is correct
      tsta
      beq cfg_endif1
cfg_loop:               ; repeat until valid input is given
      ldd #CONFIGMSG    ; display configuration message
      jsr [printf,PCr]
      jsr [getchar,PCr] ; read input from user
      stab CFG_INPUT,SP ;
      movb #TRUE,CFG_FLAG,SP; set flag as true
cfg_if2:
      ldaa CFG_INPUT,SP ; check if input is 'a'
      cmpa #'a'
      bne cfg_elseif2
      ldaa #0           ; update code at index 0
      jsr setcode
      bra cfg_endif2
cfg_elseif2:
      cmpa #'1'         ; check if input is between '1' and '4'
      blo cfg_else2
      cmpa #'4'
      bhi cfg_else2
      suba #ASCII_CONV_NUM  ; calculate index based on input
      bsr setcode           ; update the code at the calculated index
      bra cfg_endif2
cfg_else2:              ; handle invalid input
      ldd #CERRMSG      ; display error message
      jsr [printf,PCr]  ;
      movb #FALSE,CFG_FLAG,SP ; mark flag as false
cfg_endif2:
      tst CFG_FLAG,SP   ; continue loop if flag is false
      beq cfg_loop
cfg_endif1:
      leas CFG_VARSIZE,SP ; clean up stack
      puld
      rts
...
; Stack Usage
	OFFSET 0 ; to setup offsets into stack
EMC_RETVAL  DS.B 1   ; return value
EMC_I       DS.B 1   ; loop counter
EMC_INPUT   DS.B 1   ; stores input
EMC_ALARMCODE DS.W 1 ; accumulates alarm code digits
EMC_MULT    DS.W 1   ; multiplier for digit positions
EMC_VARSIZE:
EMC_PR_Y    DS.W 1   ; save register Y
EMC_PR_X    DS.W 1   ; save register X
EMC_PR_B    DS.B 1   ; save register B
EMC_RA	    DS.W 1   ; return address

enterMstCode pshb
   pshx
   pshy
   leas -EMC_VARSIZE,SP ; allocate space for variables
   movb #FALSE,EMC_RETVAL,SP  ; set initial return value to false
   movw #0,EMC_ALARMCODE,SP   ; initialize alarm code
   movw #1000,EMC_MULT,SP     ; set multiplier for first digit
